...
| On-device JS pipeline port (engine-agnostic) | **62 / 62** | `cd frontend && node --test src/lib/__tests__/` |
...
|---|---|
| **Health & Sciences** | A clinical-decision-support tool with explicit human-in-the-loop design, 6-layer anti-hallucination, strict-evidence danger-sign grounding, and a real ASHA workflow (health-center mode + field mode with later sync) — not a research demo. |
